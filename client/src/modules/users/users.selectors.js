...
export const sortUsers = (u1, u2) => {
  let comp = 0;
  if (u1.role === 'admin' && (u2.role === 'ri' || u2.role === 'pm' || u2.role === 'deputy')) {
    comp = -1;
  } else if (u1.role === 'ri' && (u2.role === 'pm' || u2.role === 'deputy')) {
    comp = -1;
  } else if (u1.role === 'ri' && u2.role === 'admin') {
    comp = 1;
  } else if (u1.role === 'pm' && (u2.role === 'admin' || u2.role === 'ri')) {
    comp = 1;
  } else if (u1.role === 'pm' && u2.role === 'deputy') {
    comp = -1;
  } else if (u1.role === 'deputy' && (u2.role === 'admin' || u2.role === 'ri' || u2.role === 'pm')) {
    comp = 1;
  }
  if (comp === 0) {
